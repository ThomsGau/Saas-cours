...
public interface AvailabilitySlotRepository extends JpaRepository<AvailabilitySlot, Long> {
    ...
    @Query("SELECT s FROM AvailabilitySlot s JOIN FETCH s.instructor WHERE s.booked = false AND s.startAt > :after ORDER BY s.startAt ASC")
    List<AvailabilitySlot> findByBookedFalseAndStartAtAfterOrderByStartAtAsc(@Param("after") Instant after);

    @Query("SELECT s FROM AvailabilitySlot s WHERE s.instructor.id = :instructorId AND s.startAt < :endAt")
    List<AvailabilitySlot> findByInstructorIdAndStartAtBefore(
            @Param("instructorId") Long instructorId,
